...
@Component({
    selector: 'rr-application-for-vehicle-form',
    templateUrl: './application-for-vehicle-form.html',
    host: {
        '[class.component]': 'true',
        '[class.load]': 'loading'
    }
})
export class ApplicationForVehicleFormComponent extends AbstractFormPage<ApplicationForVehicle> {
            ...
            buttons: [{
                ...
                click: (modal: any, event: any) => {
                    if (modal.model.comment && modal.model.comment.trim()) {
                        super.onAction(action, {
                            target: <ApplicationForVehicle>{ id: this.model.id },
