...
export class ValueObjectService {
    ...
    private fetchApprovalTypes() {
        this.translate.get(['serial', 'parallel', 'signing']).pipe(map((t: any) => [
    ...
    private fetchApprovalStatusTypes() {
        this.translate.get(['draft', 'pending', 'finished', 'awaiting', 'rejected', 'registered']).pipe(map((t: any) => [
    ...
    private fetchApprovalResultTypes() {
        this.translate.get(['accepted', 'rejected', 'project']).pipe(map((t: any) => [
    ...
    private fetchControlStatusTypes() {
        this.translate.get(['draft', 'open', 'processing', 'pending', 'completed', 'cancelled']).pipe(map((t: any) => [
    ...
    private fetchTimeLimits() {
        this.translate.get(['no', 'minutes', 'hour', 'hours', 'day', 'days', 'week']).pipe(map((t: any) => [
            { id: 0, name: t.no },
