...
public class RequestService extends RestProvider {
    ...
    private _ActionBar getActionBar(_Session session, Request request, RequestDomain requestDomain) {
        ...
        if (request.isNew()) {
            actionBar.addAction(new _Action(_ActionType.SAVE_AND_CLOSE).caption("send_request").cls("btn-primary"));
        }

        if (requestDomain.userCanDoResolution(request, (User) session.getUser())) {
            actionBar.addAction(new _Action(_ActionType.CUSTOM_ACTION).id("accept").caption("accept").cls("btn-primary"));
            actionBar.addAction(new _Action(_ActionType.CUSTOM_ACTION).id("decline").caption("decline"));
        }
