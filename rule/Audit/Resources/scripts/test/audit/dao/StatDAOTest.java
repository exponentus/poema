...
public class StatDAOTest {
	...
	private static Map<String, String> getProp() {
		...
		properties.put(PersistenceUnitProperties.JDBC_DRIVER, DataengineConst.JDBC_DRIVER);
	...
	public void findStatDataByObsStatus() {
		...
		cq.select(cb.construct(StatObsStatusDTO.class, root.get("status"), cb.count(root))).groupBy(root.get("status"));
	...
	public void findStatInspector() {
		...
		cq.select(cb.construct(StatInspectorDTO.class, root.get("author").get("login"), root.get("status"), cb.count(root)))
				.groupBy(root.get("author").get("login"), root.get("status")).orderBy(orderBy);
